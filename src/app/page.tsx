...
const s: Record<string, React.CSSProperties> = {
  ...
  badge: { fontSize: 11, fontWeight: 600, background: "#fff3ee", color: "#e85d26", padding: "4px 12px", borderRadius: 20, border: "1px solid #ffd0c0" },
  ...
  inp: { width: "100%", border: "1.5px solid #e0ddd8", borderRadius: 8, padding: "10px 14px", fontSize: 14, color: "#1a1a1a", background: "#fff", outline: "none", fontFamily: "inherit", resize: "vertical" },
  ...
  advice: { background: "#f9f8f6", border: "1px solid #ede9e3", borderRadius: 8, padding: 16, fontSize: 13, lineHeight: 2, whiteSpace: "pre-wrap" },
  uploaderArea: { border: "2px dashed #e0ddd8", borderRadius: 8, padding: "20px", textAlign: "center", cursor: "pointer", background: "#f9f8f6", transition: "border-color 0.2s" },
};
          ...
          <>
            ...
            <div style={s.grid}>
              ...
              <div style={s.card}>
                ...
                <div style={{ marginBottom: 14 }}>
                  ...
                  <div style={{ display: "flex", gap: 6, marginBottom: 8 }}>
                    <input
                      style={{ ...s.inp, flex: 1 }}
                      placeholder="例：Python, Django（Enterで追加）"
                      value={reqSkillInput}
                      onChange={e => setReqSkillInput(e.target.value)}
                      onKeyDown={e => { if (e.key === "Enter") { e.preventDefault(); addSkill("required", reqSkillInput); } }}
                    />
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                    {job.required_skills.map((sk, i) => (
                      <span key={i} style={s.tag} onClick={() => removeSkill("required", i)} title="クリックで削除">
                        {sk} ×
                      </span>
                    ))}
                  </div>
                ...
                <div style={{ marginBottom: 14 }}>
                  ...
                  <div style={{ display: "flex", gap: 6, marginBottom: 8 }}>
                    <input
                      style={{ ...s.inp, flex: 1 }}
                      placeholder="例：AWS, Docker（Enterで追加）"
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                    {job.preferred_skills.map((sk, i) => (
                      <span key={i} style={{ ...s.tag, background: "#f5f5f0", color: "#888", border: "1px solid #e0ddd8" }} onClick={() => removeSkill("preferred", i)} title="クリックで削除">
                        {sk} ×
                      </span>
                    ))}
                  </div>
              ...
              <div style={s.card}>
                ...
                <div style={s.cardDesc}>Career BuilderのCSVをアップロード、または手入力</div>

                {/* CSV アップロード */}
                <div
                  style={{ ...s.uploaderArea, borderColor: dragging ? "#e85d26" : "#e0ddd8", marginBottom: 16 }}
                ...
                >
                  ...
                  <div style={{ fontSize: 13, fontWeight: 600, color: "#555" }}>Career BuilderのCSVをドロップ</div>
                  <div style={{ fontSize: 11, color: "#bbb", marginTop: 4 }}>またはクリックして選択</div>
                  {career.name && (
                    <div style={{ marginTop: 8, fontSize: 12, color: "#e85d26", fontWeight: 600 }}>✓ {career.name} のデータを読み込みました</div>
                  )}
                </div>

                <div style={{ marginBottom: 14 }}>
                  <label style={s.label}>技術スタック</label>
                  <div style={s.hint}>カンマ区切りで入力（CSVから自動入力されます）</div>
                ...
                <div style={{ marginBottom: 14 }}>
                  ...
                  <textarea style={{ ...s.inp, minHeight: 60 }} placeholder="顧客折衝・PM経験・プロジェクト管理など" value={career.summary_consulting} onChange={e => setCareer(p => ({ ...p, summary_consulting: e.target.value }))} />
                </div>
            ...
            <div style={{ textAlign: "center" }}>
              <button
                style={{ ...s.btn, ...s.btnPrimary, minWidth: 240, opacity: loading ? 0.7 : 1 }}
                onClick={handleMatch}
                disabled={loading}
              >
                {loading ? "診断中..." : "🔍　マッチング診断を実行"}
              </button>
              {loading && <div style={{ fontSize: 12, color: "#aaa", marginTop: 8 }}>BERTでベクトル化・類似度計算中です（10〜30秒程度）</div>}
            </div>
          </>
        )}
          ...
          <>
            ...
            <div style={{ ...s.card, marginBottom: 16 }}>
              ...
              <div style={{ display: "flex", justifyContent: "center", gap: 48, flexWrap: "wrap" }}>
                <ScoreGauge score={result.score_sbert} label="BERT類似度" />
                {result.score_w2v !== null && (
                  <ScoreGauge score={result.score_w2v} label="Word2Vec類似度" />
                )}
                <ScoreGauge
                  score={Math.max(0, 1 - result.missing_skills.length * 0.12)}
                  label="スキルカバー率"
                  color="#7c3aed"
                />
              </div>
            ...
            <div style={{ ...s.card, marginBottom: 16 }}>
              ...
              {result.missing_skills.length === 0 ? (
                <div style={{ color: "#16a34a", fontWeight: 600, fontSize: 14 }}>✓ 不足スキルは検出されませんでした</div>
              ) : (
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                  {result.missing_skills.map((sk, i) => (
                    <span key={i} style={s.tagMissing}>⚠ {sk}</span>
                  ))}
                </div>
              )}
            </div>
            ...
            <div style={{ display: "flex", gap: 10, justifyContent: "center" }}>
              <button style={{ ...s.btn, ...s.btnPrimary }} onClick={() => { setStep("input"); setResult(null); }}>
                別の求人で診断
              </button>
              <button style={{ ...s.btn, ...s.btnGhost }} onClick={() => window.print()}>
                🖨 印刷・PDF保存
              </button>
            </div>
          </>
