...
const initialData = {
  ...
  working: { rate: "", rateUnit: "月額", days: "", remote: "", location: "", available: "", jobType: [] as string[] },
};
...
export default function CareerBuilderPage() {
  ...
  const exportCSV = () => {
    const rows = [
      ...
      ["稼働条件", "希望単価", `${data.working.rate}（${data.working.rateUnit}）`],
      ["稼働条件", "稼働日数・時間", data.working.days],
              ...
              <div style={{ ...s.grid2, marginBottom: 14 }} className="grid2">
                <div>
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    <input style={{ ...s.inp, flex: 1 }} placeholder="例：700,000" value={data.working.rate} onChange={e => update("working", "rate", e.target.value)} />
                    <select style={{ ...s.inp, width: 74, padding: "10px 8px" }} value={data.working.rateUnit} onChange={e => update("working", "rateUnit", e.target.value)}>
                      <option>月額</option><option>時給</option><option>日額</option>
                    </select>
                  </div>
                </div>
                <div>
                  <label style={s.label}>稼働可能日数・時間</label>
                  <input style={s.inp} placeholder="例：週3日・1日8時間" value={data.working.days} onChange={e => update("working", "days", e.target.value)} />
                </div>
                <div>
